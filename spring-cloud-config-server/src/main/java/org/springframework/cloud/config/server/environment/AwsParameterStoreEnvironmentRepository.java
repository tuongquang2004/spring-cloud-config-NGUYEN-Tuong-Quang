 ...
 */
 ...
 public class AwsParameterStoreEnvironmentRepository implements EnvironmentRepository, Ordered {
	 ...
	 public AwsParameterStoreEnvironmentRepository(SsmClient awsSsmClient,
	 ...
	 public Environment findOne(String application, String profile, String label) {
		 return findOne(application, profile, label, true); // Add a parametre
	 }
 
	 public Environment findOne(String application, String profile, String label, boolean includeOverrides) {
		 ...
		 List<PropertySource> propertySources = getPropertySources(paths, includeOverrides);
	 ...
	 private List<PropertySource> getPropertySources(Set<String> parameterPaths, boolean includeOverrides) {
		 ...
		 if (response != null) {
			 ...
			 while (StringUtils.hasLength(response.nextToken())) {
				 response = awsSsmClient.getParametersByPath(
					 request.toBuilder().nextToken(response.nextToken()).build()
				 );
				 addParametersToProperties(path, response.parameters(), result);
			 }
		 }
	 ...
	 private void addParametersToProperties(String path, List<Parameter> parameters, Map<String, String> properties) {
		 for (Parameter parameter : parameters) {
			 String name = StringUtils.delete(parameter.name(), path)
